...
function PlanCard({ plan: p }: { plan: typeof plans[0] }) {
  return (
    <div
      style={{ background: p.featured ? "var(--green)" : "var(--paper)", padding: "36px 30px", borderRight: "1px solid var(--border)", transition: "background .2s" }}
      onMouseEnter={e => { if (!p.featured) e.currentTarget.style.background = "var(--cream)"; }}
      onMouseLeave={e => { if (!p.featured) e.currentTarget.style.background = "var(--paper)"; }}
    >
